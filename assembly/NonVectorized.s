...
inner_butterfly_loop:
 bge s9, s6, end_inner_butterfly # if j >= m/2, exit butterfly pairs loop

 # Compute twiddle factor index
 mul s10, s9, s8 # s10 = j * twiddle_stride
 slli s11, s10, 2 # Multiply by 4 to get byte offset

 # Load twiddle factors
 add t1, s2, s11 # Address of twiddle_real[j * stride]
 flw ft0, 0(t1) # ft0 = wr = twiddle_real[j * stride]
 
 add t1, s3, s11 # Address of twiddle_imag[j * stride]
 flw ft1, 0(t1) # ft1 = wi = twiddle_imag[j * stride]

 # Compute indices for butterfly operations
 add t1, t0, s9 # t1 = i + j (top butterfly index)
 slli t2, t1, 2 # Byte offset = (i + j) * 4
 
 add t3, t1, s6 # t3 = i + j + m/2 (bottom butterfly index)
 slli t4, t3, 2 # Byte offset = (i + j + m/2) * 4

 # Load top butterfly values
 add t5, s0, t2 # Address of rev_real[i + j]
 flw ft2, 0(t5) # ft2 = a_real = rev_real[i + j]
 
 add t5, s1, t2 # Address of rev_imag[i + j]
 flw ft3, 0(t5) # ft3 = a_imag = rev_imag[i + j]

 # Load bottom butterfly values
 add t5, s0, t4 # Address of rev_real[i + j + m/2]
 flw ft4, 0(t5) # ft4 = b_real = rev_real[i + j + m/2]
 
 add t5, s1, t4 # Address of rev_imag[i + j + m/2]
 flw ft5, 0(t5) # ft5 = b_imag = rev_imag[i + j + m/2]

 # Calculate twiddle * bottom half
 # (ft6, ft7) = (wr,wi) * (b_real,b_imag) = (wr*b_real - wi*b_imag, wr*b_imag + wi*b_real)
 fmul.s ft8, ft4, ft0  # ft8 = b_real * wr
 fmul.s ft9, ft5, ft1  # ft9 = b_imag * wi
 fsub.s ft6, ft8, ft9  # ft6 = twiddle_real_part = b_real*wr - b_imag*wi
 
 fmul.s ft8, ft5, ft0  # ft8 = b_imag * wr
 fmul.s ft9, ft4, ft1  # ft9 = b_real * wi
 fadd.s ft7, ft8, ft9  # ft7 = twiddle_imag_part = b_imag*wr + b_real*wi

 # Calculate butterfly results
 # Top half: (a_real,a_imag) + (twiddle_real,twiddle_imag)
 fadd.s ft8, ft2, ft6  # ft8 = new_a_real = a_real + twiddle_real
 fadd.s ft9, ft3, ft7  # ft9 = new_a_imag = a_imag + twiddle_imag

 # Bottom half: (a_real,a_imag) - (twiddle_real,twiddle_imag)
 fsub.s ft10, ft2, ft6 # ft10 = new_b_real = a_real - twiddle_real
 fsub.s ft11, ft3, ft7 # ft11 = new_b_imag = a_imag - twiddle_imag

 # Store results back
 add t5, s0, t2        # Address of rev_real[i + j]
 fsw ft8, 0(t5)        # Store new_a_real
 
 add t5, s1, t2        # Address of rev_imag[i + j]
 fsw ft9, 0(t5)        # Store new_a_imag
 
 add t5, s0, t4        # Address of rev_real[i + j + m/2]
 fsw ft10, 0(t5)       # Store new_b_real
 
 add t5, s1, t4        # Address of rev_imag[i + j + m/2]
 fsw ft11, 0(t5)       # Store new_b_imag
